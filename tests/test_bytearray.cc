...
#define XX(type, len, write_fun, read_fun, base_len)                                  \
    {                                                                                 \
        ...
        SYLAR_LOG_INFO(g_logger) << #write_fun "/" #read_fun                          \
                                 ...
                                 << " size=" << ba->getSize();                        \
        ba->setPosition(0);                                                           \
        SYLAR_ASSERT(ba->writeToFile("/tmp/a" #type "_" #len "-" #read_fun ".dat"));  \
        sylar::ByteArray::ptr ba2(new sylar::ByteArray(base_len * 2));                \
        SYLAR_ASSERT(ba2->readFromFile("/tmp/a" #type "_" #len "-" #read_fun ".dat"));\
        ba2->setPosition(0);                                                          \
 ...
 */
// random_shuffle是一种常见的随机化函数，它能够将容器（如数组或列表）中的元素顺序随机打乱。
    SYLAR_LOG_INFO(g_logger) << "test3: ";
