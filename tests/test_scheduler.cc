...
void test_fiber5() 
{
    SYLAR_LOG_INFO(g_logger) <<"current task_count: " <<sylar::Scheduler::GetThis()->getTaskCount();
    static int count = 0;
...
int main() 
{
     ...
     * 消耗掉，然后再返回main函数往下执行
     */
    // 构造函数中的参数都有默认值，线程数量为1， use_caller为true，线程名为scheduler
    sylar::Scheduler sc(1, true, "Scheduler"); 
    ...
    /**
     * 只要调度器未停止，就可以添加调度任务
     * 包括在子协程中也可以通过sylar::Scheduler::GetThis()->scheduler()的方式继续添加调度123456任务
     */
