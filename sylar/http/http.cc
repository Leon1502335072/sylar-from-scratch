...
// 字符串数组，s_method_string[DELETE]=DELETE, s_method_string[GET]=GET 前一个GET是数字下标，后一个GET是字符串
...
HttpRequest::HttpRequest(uint8_t version, bool close)
    ...
    , m_version(version)
    , m_close(close) //默认是true
    , m_websocket(false)
    , m_parserParamFlag(0)
    , m_path("/") {
    // 参数列表给了很多默认值，且不是通过形参给的
}
...
{
    // header集合中的key是忽略大小写的，所以connection和Connection一样
...
HttpResponse::HttpResponse(uint8_t version, bool close)
    ...
    , m_websocket(false) {
    // 构造中给m_status的默认值是OK
    // version如果外部没传，也会有默认值0x11
    // m_close如果外部没传，也会有默认值true
}
...
std::ostream &HttpResponse::dump(std::ostream &os) const 
{
    ...
    // 不是服务端监听的socket而是为客户端连接而产生的socket
    if (!m_websocket) 
    {
        os << "connection: " << (m_close ? "close" : "keep-alive") << "\r\n";
    }
